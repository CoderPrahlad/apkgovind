...
import { Menu, X, Download, Globe, Shield, Zap, Headphones, Smartphone, CheckCircle2, ArrowRight, Database, Server, Code2 } from 'lucide-react'
...
export default function MatkaKingLandingPage() {
  ...
  // ── Download Handler (Live Admin Count + Direct File Download) ──
  const handleDownload = async (e: React.MouseEvent, source: string = 'landing_page') => {
    e.preventDefault(); 
    console.log("🚀 Download started from source:", source);
    
    try {
      // Live server (matkaking.store) ko batao ki download hua hai
      const response = await fetch('https://matkaking.store/api/track-download', {
        method: 'POST',
        headers: { 
      ...
      console.log("✅ Admin count updated. Status:", response.status);
    } catch (err) {
      console.error("❌ Admin update silent fail:", err);
    } finally {
      // ⬇️ Asli APK file download shuru karo
      const link = document.createElement('a');
      link.href = '/matkaking.apk'; 
      link.download = 'MatkaKing_App.apk';
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#020817] text-white overflow-x-hidden font-sans">
      ...
      <main className="flex-1 relative z-10">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-20 lg:py-24 grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
          ...
          >
            ...
            >
              ...
              <motion.button
                onClick={(e) => handleDownload(e, 'hero_button')}
        ...
        <Section id="architecture" className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          ...
          >
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black">
              React + <span className="text-emerald-400">Node.js</span> + <span className="text-cyan-400">MySQL</span>
            </h2>
            <p className="text-gray-400 mt-3 sm:mt-4 text-sm sm:text-lg max-w-2xl mx-auto">
              Built with a modern, scalable architecture separating frontend, backend, and database layers.
            </p>
          </motion.div>
          ...
          >
            <motion.div
              ...
              className="bg-white/5 border border-white/10 rounded-2xl sm:rounded-3xl p-6 sm:p-7 backdrop-blur-xl hover:bg-white/[0.07] transition-colors duration-300 group"
            >
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-500 to-green-400 flex items-center justify-center text-black mb-5 group-hover:scale-110 transition-transform duration-300">
                <Code2 className="w-8 h-8" />
              </div>
              <h3 className="text-xl sm:text-2xl font-black">React Frontend</h3>
              <p className="text-gray-400 mt-2 sm:mt-3 leading-relaxed text-xs sm:text-sm">
                React 19 with Vite, Tailwind CSS 4, and shadcn/ui components. Client-side SPA with Framer Motion animations and React Router for navigation.
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                {['React 19', 'Vite 6', 'Tailwind CSS', 'shadcn/ui'].map((tag) => (
                  <span key={tag} className="bg-emerald-500/10 text-emerald-300 px-2 py-1 rounded-md text-xs font-medium">{tag}</span>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="bg-white/5 border border-white/10 rounded-2xl sm:rounded-3xl p-6 sm:p-7 backdrop-blur-xl hover:bg-white/[0.07] transition-colors duration-300 group"
            >
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-cyan-500 to-blue-400 flex items-center justify-center text-black mb-5 group-hover:scale-110 transition-transform duration-300">
                <Server className="w-8 h-8" />
              </div>
              <h3 className="text-xl sm:text-2xl font-black">Node.js Backend</h3>
              <p className="text-gray-400 mt-2 sm:mt-3 leading-relaxed text-xs sm:text-sm">
                Express.js REST API with Socket.IO for real-time WebSocket communication. Prisma ORM for type-safe database access and migrations.
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                {['Express 5', 'Socket.IO', 'Prisma ORM', 'REST API'].map((tag) => (
                  <span key={tag} className="bg-cyan-500/10 text-cyan-300 px-2 py-1 rounded-md text-xs font-medium">{tag}</span>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="bg-white/5 border border-white/10 rounded-2xl sm:rounded-3xl p-6 sm:p-7 backdrop-blur-xl hover:bg-white/[0.07] transition-colors duration-300 group"
            >
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-400 to-teal-400 flex items-center justify-center text-black mb-5 group-hover:scale-110 transition-transform duration-300">
                <Database className="w-8 h-8" />
              </div>
              <h3 className="text-xl sm:text-2xl font-black">MySQL Database</h3>
              <p className="text-gray-400 mt-2 sm:mt-3 leading-relaxed text-xs sm:text-sm">
                MySQL 8 with Prisma migrations for reliable data persistence. User and Post models with full CRUD operations and relational integrity.
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                {['MySQL 8', 'Prisma Migrate', 'CRUD API', 'Relations'].map((tag) => (
                  <span key={tag} className="bg-teal-500/10 text-teal-300 px-2 py-1 rounded-md text-xs font-medium">{tag}</span>
                ))}
              </div>
            </motion.div>
          </motion.div>
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="mt-8 sm:mt-10 bg-white/[0.03] border border-white/10 rounded-2xl sm:rounded-3xl p-5 sm:p-6 backdrop-blur-xl"
          >
            <h4 className="text-lg sm:text-xl font-bold text-emerald-400 mb-4">API Endpoints</h4>
              ...
              {[
                ...
                <div key={i} className="flex items-center gap-2 bg-black/30 rounded-lg px-3 py-2">
                  <span className={`font-black text-xs px-1.5 py-0.5 rounded ${
                    endpoint.method === 'GET' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-cyan-500/20 text-cyan-400'
                  }`}>
                    {endpoint.method}
                  </span>
                  <span className="text-gray-300 text-xs truncate">{endpoint.path}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <div className="max-w-5xl mx-auto rounded-[32px] sm:rounded-[40px] overflow-hidden border border-white/10 bg-gradient-to-r from-emerald-500/20 via-cyan-500/10 to-black p-8 sm:p-12 text-center backdrop-blur-2xl relative">
            ...
            >
              ...
              <div className="flex flex-wrap justify-center gap-4 sm:gap-5 mt-8 sm:mt-10">
                ...
                <motion.button
                  onClick={(e) => handleDownload(e, 'bottom_section')}
                  whileHover={{ scale: 1.03 }}
                  whileTap={{ scale: 0.97 }}
