...
// ─── Links ───────────────────────────────────────────────────
const WEBSITE_LINK = 'https://matkaking.store/'
...
export default function HomePage() {
  ...
  // ── Download Handler (X-Ray Debugging Mode) ──
  const handleDownload = async (e: React.MouseEvent, source: string = 'landing_page') => {
    e.preventDefault(); 
    console.log("🚀 Step 1: Download Button Click hua! Source:", source);
    
    try {
      console.log("📡 Step 2: Live Admin Panel ko request bhej rahe hain...");
      ...
      console.log("✅ Step 3: Server ka response aa gaya! Status code:", response.status);
      
      const data = await response.json().catch(() => null);
      console.log("📦 Step 4: Server ne yeh data bheja:", data);

    } catch (err) {
      console.error("❌ ERROR: Request raste mein hi fail ho gayi!", err);
    } finally {
      console.log("⬇️ Step 5: File download start kar rahe hain...");
    ...
    <div className="min-h-screen flex flex-col bg-[#020817] text-white overflow-x-hidden font-sans">
      ...
      <main className="flex-1 relative z-10">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-20 lg:py-24 grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
          ...
          {/* App Mockup */}
          <motion.div
            initial={{ opacity: 0, x: 60, rotateY: -10 }} animate={{ opacity: 1, x: 0, rotateY: 0 }}
                ...
                <div className="bg-[#031320] rounded-[32px] overflow-hidden border border-white/10">
                  <div className="bg-gradient-to-r from-emerald-500 to-cyan-500 p-5 sm:p-6 text-black">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-black/20 flex items-center justify-center font-black text-xl sm:text-2xl">MK</div>
                    </div>
                  </div>
                  <div className="p-4 sm:p-6 space-y-4 sm:space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:gap-3">
                      ...
                      <motion.button whileHover={{ scale: 1.03, backgroundColor: 'rgba(255,255,255,0.15)' }} whileTap={{ scale: 0.97 }}
          ...
          >
            {features.map((item, index) => (
              ...
              >
                ...
                <p className="text-gray-400 mt-2 sm:mt-3 leading-relaxed text-xs sm:text-sm">{item.desc}</p>
              </motion.div>
            ))}
          </motion.div>
        </Section>
        ...
        <Section id="trust" className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
          <div className="bg-white/[0.03] border border-white/10 rounded-3xl sm:rounded-[40px] p-8 sm:p-12 backdrop-blur-xl">
            <div className="grid sm:grid-cols-3 gap-6 sm:gap-8 text-center">
              <div>
                <h3 className="text-4xl sm:text-5xl font-black text-emerald-400"><AnimatedCounter target={50} suffix="K+" /></h3>
                <p className="text-gray-400 mt-2 text-sm sm:text-base">Active Users</p>
              </div>
              <div>
                <h3 className="text-4xl sm:text-5xl font-black text-cyan-400"><AnimatedCounter target={99} suffix="%" /></h3>
                <p className="text-gray-400 mt-2 text-sm sm:text-base">Uptime Guarantee</p>
              </div>
              <div>
                <h3 className="text-4xl sm:text-5xl font-black text-emerald-400"><AnimatedCounter target={24} suffix="×7" /></h3>
          ...
          <div className="max-w-5xl mx-auto rounded-[32px] sm:rounded-[40px] overflow-hidden border border-white/10 bg-gradient-to-r from-emerald-500/20 via-cyan-500/10 to-black p-8 sm:p-12 text-center backdrop-blur-2xl relative">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative z-10">
              ...
              <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black leading-tight">
                DOWNLOAD<br />
                <span className="bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent">MATKA KING APK</span>
              </h2>
        ...
        <Section id="support" className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
          <div className="bg-white/[0.03] border border-white/10 rounded-2xl sm:rounded-3xl p-6 sm:p-8 backdrop-blur-xl text-center">
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-6">
              <motion.a href="https://t.me/coderprahlad/286" target="_blank" rel="noopener noreferrer"
      ...
      <footer className="relative z-10 border-t border-white/10 mt-6 sm:mt-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10 flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6">
          <div className="text-center md:text-left">
            <h3 className="text-2xl sm:text-3xl font-black">Matka<span className="text-emerald-400">King</span></h3>
            <p className="text-gray-400 text-xs sm:text-sm mt-1 sm:mt-2">Premium Number Game Platform</p>
          </div>
